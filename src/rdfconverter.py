...
if os.path.exists(args.mapping[0]):
    with open(args.mapping[0],"r") as f:
        typemap=json.load(f)
with open(str(args.output[0])+"/"+str(path[0:path.rfind(".")])+"_"+str(args.mapping[0][0:args.mapping[0].rfind(".")]).replace("/","_")+"_autotypemap.json","w") as f:
...
g.serialize(str(args.output[0])+"/"+path[0:path.rfind(".")]+"_"+str(args.mapping[0][0:args.mapping[0].rfind(".")]).replace("/","_")+".ttl",format="turtle")
